...
- **Config import no longer breaks the database** — the import wrote its timestamps in a format SQLite stored happily but could never read back, so it reported success and then every request touching an imported row failed with HTTP 500: peers, users and avatars all gone. The import now writes the format the driver reads, and a database already damaged by an earlier import is repaired automatically on the next start — no re-install, no restore. **If you have ever used config import, take this release.**
