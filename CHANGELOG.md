...
- **The heap is capped, so the memory figure is a constant instead of a share of the machine.** `-Xmx192m` in the systemd unit, overridable with `ISLANDR_MAX_HEAP`. Without a cap a native image takes up to 80% of the host's RAM as its maximum heap and the collector has no reason to hand any back — on a 1 GB VPS that read as 253 MB resident for an idle service, and on a larger host it would have read as something else again.
- **The install guide leads with the three commands that install it.** The step-by-step walkthrough moved to [docs/install/manual.md](docs/install/manual.md) — `install.md` is the path, that page is the reference for a host that is not a fresh Debian/Ubuntu VPS.
...
- **Fixed: a webhook's last delivery status could stay stale without a trace.** When saving it collided with another database access, the error was swallowed; it is retried now and logged if it keeps failing.
- **My access shows a device's configuration even when its private key is not stored** — without the `PrivateKey` line and without a QR code, but with address, DNS, endpoint and allowed IPs current. It used to answer "no .conf stored" and hide the button under retention `never`.
- **Fixed: "encrypted" key retention without a loaded key failed every new device with a bare HTTP 500.** Settings now refuse the mode while no key is loaded, and an instance already in that state answers 503 with the reason instead of an empty error.
- **The sign-in screen has a night sky in dark mode** — the same scene as the website's hero: stars turning once every twelve minutes, islands on the horizon, the tunnels between them. It stops under reduced motion. The wordmark, which had a fixed dark fill, is readable on it now.
