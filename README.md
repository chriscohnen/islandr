...
**0.14.0**
- **Let's Encrypt, fully automatic** — set a domain in Settings and islandr requests, validates, and installs a real certificate itself, then renews it before it expires — no Certbot, no cron job, no reverse proxy. The ACME client (RFC 8555) is hand-rolled against the JDK's own HTTP and crypto APIs rather than pulling in a certificate library, keeping the native-image build free of a whole class of reflection risk ([ADR-0019](docs/adr/0019-acme-hand-rolled-client.md), [#30](https://github.com/chriscohnen/islandr/issues/30)). The TLS Settings card is now split into Let's Encrypt / Origin Server Certificate tabs instead of one stacked block.
- **Device discovery now actually scans by default** — `islandr.discovery.mode` silently defaulted to `mock` with no mention in the install docs, so a standard install's "Find devices" permanently returned two hardcoded fake hosts instead of ever touching the real network. Now defaults to `real` (an unprivileged unicast probe needs no `sudo`, unlike `wg`/`nft`, so there's no reason to require opt-in); `mock` is still available by setting `ISLANDR_DISCOVERY_MODE=mock`. The fixed probe-port set also grew two entries: `7222` (TIBCO EMS) and `9443` (common alt-HTTPS for admin UIs like Portainer/Proxmox).
...
- **See what certificate is actually installed** — Settings now shows the active certificate's domain, SANs, validity window, and issuer, instead of just an expiry date.
- **TLS upload errors now say why** — a bad paste used to come back as a bare "HTTP 400" with nothing in the log; the actual reason now reaches the UI.
- **Connected peers are labelled on the topology diagram** — a live peer used to be an unlabeled dot near the hub; it now shows its name and IP with a connecting line.
- **New installs get a working encryption key out of the box** — the installer and Docker Compose setup now generate the private-key-retention encryption key automatically, and the native installer takes a `WG_INTERFACE` override for hosts not using `wg0`.
...
- **The enforcement banner no longer overflows.** In degraded mode the long "from source" install command ran off the right edge of the banner (pushing its copy button out of view) instead of scrolling within its own line; it — and any wide content in a callout — now stays inside the box. The stylesheet is now revalidated on load like the rest of the app code, so UI fixes reach you without a manual hard-refresh.
- **The sidebar stays put.** On long pages the navigation used to scroll away with the content, so "Settings" at its foot was easy to miss; the rail is now pinned to the viewport and always reachable.
- **The seeded port groups are English.** The five default port groups shipped with German names and descriptions ("Drucker_Standard_Ports", "Windows-Dateifreigabe", …); they now read in English. Groups you have renamed or edited are left untouched.
