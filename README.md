...
- **Onboarding polish** — the port form pre-fills the default port for the chosen protocol (RDP 3389, SSH 22, HTTPS 443, …); creating a peer with no users yet still allows a site gateway (client is disabled with a hint to add a user first); networks show a hint when there is no site peer to pick as gateway.
