...
- **A failed login writes a line fail2ban can match**, carrying the client address and how long that attempt was held (`delay=250ms`), with both on the audit row too. Behind a proxy, read [docs/install/fail2ban.md](docs/install/fail2ban.md) first: banning the peer address bans your proxy, and trusting `X-Forwarded-For` blindly lets an attacker ban a third party instead — so the header counts only for proxies you name under **Settings → Reverse proxy**, and that list is empty by default. `setup-hub.sh` takes `TRUSTED_PROXIES=` to seed it at install time; the guide carries a worked Traefik example
- **Discovery says which network has no DNS server, and lets you set one where you noticed it** — the network's own DNS server is the only source that returns names someone registered, and on a network reached through a gateway it is the only one that can work at all. Left empty, a `/24` imports as a column of `computer-42`. The field is now settable inside the scan dialog, before and after a scan, and once per gateway in the network import ([#74](https://github.com/chriscohnen/islandr/issues/74))
- **Fixed: the downloaded `.rdp` ignored the port's clipboard setting** — the switch was enforced for the browser session but the file for the native client hardcoded clipboard redirection on, so the console said off and the client said on. The file follows the setting now, and the form states what that is worth: enforced in the browser, a default in a file the user can edit for the native client
- **Fixed: peers with no IPv4 were offered for an import that could not succeed** — a peer on the interface carrying only an IPv6 address was selectable, with an empty IP column and no explanation, and importing it always failed. It is still listed, now labelled "no IPv4" and not selectable ([#86](https://github.com/chriscohnen/islandr/issues/86) tracks making it importable)
