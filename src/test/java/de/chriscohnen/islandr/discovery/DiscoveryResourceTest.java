...
class DiscoveryResourceTest {
    ...
    /** Same ResourceTypes.PATTERN the console's own resource create/update
     *  endpoint validates against (AclEndpointsTest) — a type added there is
     *  importable here too, not just creatable by hand. */
    @Test
    void import_acceptsAccesspointType() {
        String siteId = createSite("disco-ap", "10.96.0.0/29");
        String body = "{\"hosts\":[{\"ip\":\"10.96.0.5\",\"name\":\"ap-1\",\"type\":\"accesspoint\"}]}";
        ...
        given().when().get("/api/v1/sites/" + siteId + "/resources")
                .then().statusCode(200).body("find { it.ip == '10.96.0.5' }.type", equalTo("accesspoint"));
    }
