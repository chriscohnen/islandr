...
- **Network diagnostics stay inside the unprivileged-shell model and never accept a free-text target** (T-018) — the ping/tracepath probe adds no `CAP_NET_RAW`, no new Linux capability, only a `sudoers` entry scoped to the exact `ping -c 4 -W 2 *` invocation shape (`tracepath` needs no elevation at all on Linux). The target is always resolved from an existing `Resource`, admin-only, rate-limited per resource, and audit-logged ([ADR-0025](../adr/0025-network-diagnostic-helpers.md)).
