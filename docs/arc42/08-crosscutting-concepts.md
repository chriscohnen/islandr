...
| Discovery preconditions unmet (real scan whose declared tunnel gateway is stale, CIDR > /22, scan already running) | Rejected **before any probe is sent** with HTTP 409 and copy naming the specific cause. A site with no gateway peer is hub-local and allowed; mock mode skips the route check | Operator reconnects the site gateway, narrows the CIDR, or waits for the running scan (T-014) |
