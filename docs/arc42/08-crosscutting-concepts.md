...
- **No privileged Docker in production** — `--cap-add NET_ADMIN` + `--network host` is an explicitly rejected pattern (T-011). v1 Docker is mock-only. v2 introduces a Unix socket proxy ([ADR-0012](../adr/0012-docker-socket-proxy.md)) that gives the container a narrow, auditable, allowlisted channel to the host kernel — without any Linux capabilities.
