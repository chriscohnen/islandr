...
| Discovery preconditions unmet (no gateway peer, no recent handshake, CIDR > /22, scan already running) | Rejected **before any probe is sent** with HTTP 409 and copy naming the specific cause | Operator connects the site gateway, narrows the CIDR, or waits for the running scan (T-014) |
