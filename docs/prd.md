...
- **F-21** — Where a site has a connected gateway peer, an admin shall be able to run a device-discovery scan of that site's own CIDR and bulk-import the discovered hosts as resources, instead of adding each host by hand. The scan shall establish host liveness using unprivileged sockets only (TCP `connect()` to a fixed probe-port set plus a connected-`DatagramSocket` ICMP port-unreachable probe) — it shall not use raw sockets, `CAP_NET_RAW`, or any new privilege ([ADR-0014](adr/0014-device-discovery.md), [BR-032](spec.md)). The scan shall be admin-only, bound to the site's declared CIDR (no free-text range), rejected for a CIDR larger than `/22` ([BR-033](spec.md)), guarded by preconditions with clear errors ([BR-034](spec.md)), and audited on start and on import ([BR-037](spec.md)). Each discovered host shall carry its open ports and a pre-filled, editable resource-type guess ([BR-035](spec.md)); import shall be idempotent on `(site, ip)` ([BR-036](spec.md)). Discovery is best-effort — a fully-filtered host may not be found, and manual add remains available.
