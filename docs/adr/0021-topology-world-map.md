...
- Map view is additive: a tab/toggle alongside the existing radial diagram (mirroring the Settings TLS tab pattern already used elsewhere), not a replacement. **Visibility threshold, revised 2026-07-26** (the first real-world install report): the tab appears once **hub coordinates are set AND at least 1 site-peer has coordinates** — not the originally planned "≥2 site-peers, hub not counted." The original threshold undercounted a common, entirely valid topology (one hub, one remote site) as "not enough to show a map," when a hub-to-single-site line is already a meaningful picture. The hub is not optional under the revised rule (a lone geocoded site with no hub has nothing to connect to and reads as an arbitrary dot on a map), but a second site is no longer required.
