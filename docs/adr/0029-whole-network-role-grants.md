...
- **Exclusive-capacity carve-out (2026-09-05, post-merge fix).** A network grant's broad accept would otherwise silently reopen any exclusive-capacity port (issue #72) on any resource in the granted site, regardless of reservation state — nftables has no "most specific rule wins" semantics, so the broad rule would simply out-match the reservation gate's normal "no accept rule emitted" mechanism. `RuleBuilder`'s network-grant block now emits an explicit narrow `drop` for `(peer, gated port)` pairs the peer hasn't reserved, inserted before that peer's broad accept rule in the same emission pass, restoring the reservation gate for anyone who reaches a gated port through a network grant.
