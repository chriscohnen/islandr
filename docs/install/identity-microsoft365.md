...
> one. Pasting it produces a generic failure that does not mention the secret
> at all.
...
Two things about that link are worth knowing before you click it.

**It reports success as an error.** The link sends the browser back to
Islandr's OIDC callback, but an admin-consent return is not a login: it carries
`admin_consent=True` and no `code`, and it has no `state` cookie because no
login was ever started. The callback rejects it with
`state mismatch (CSRF protection)`. **The consent itself succeeded** — this is
Islandr mishandling the return, tracked as
[#81](https://github.com/chriscohnen/islandr/issues/81). Ignore the error and
try signing in.

**It may not be needed.** If the app registration already shows the
permissions as granted under **API permissions**, consent is done and the link
...
| Symptom | Cause |
|---|---|
| `AADSTS7000215: Invalid client secret provided` | The Secret ID was pasted instead of the secret Value, or the secret has expired. Create a new secret and copy the Value. |
...
| `state mismatch (CSRF protection)` right after clicking the consent link | Expected, see above — consent worked. |
