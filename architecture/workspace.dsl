    ...
    model {
        ...
        islandr = softwareSystem "Islandr" "Self-hosted WireGuard management platform. Peer lifecycle, RBAC access control, nftables enforcement." {
            ...
            backend = container "Islandr Backend" "REST API, domain logic, WireGuard and nftables adapters, OIDC token verification, activity polling, audit logging." "Quarkus 3 / Java 21 (native binary)" "Backend" {
                ...
                discoveryPkg = component "discovery" "Admin-triggered device discovery: enumerates a site's CIDR, probes host liveness with unprivileged sockets, fingerprints a resource type from open ports, names an open port from a bundled IANA-derived table (classpath only, never over the network), bulk-imports the reviewed selection as Resource rows (ADR-0014)." "JAX-RS + CDI"
                ...
                adminPkg     = component "admin"     "Instance config export/import, seeded-data timestamp repair on import, version/update-check endpoint, and whether an update.sh backup exists to roll back to (read-only; no new privilege)." "JAX-RS + CDI"
            }
